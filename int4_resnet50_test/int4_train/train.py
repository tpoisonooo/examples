...
def main():
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="number of epochs to warmup (default: 5)",
    )
    parser.add_argument(
        "-b",
        "--batch-size",
...
def worker(args):
    ...
    if dist.get_rank() == 0:
        os.makedirs(os.path.join(args.save, args.arch, args.mode), exist_ok=True)
    ...
    start_step = 0
    continue_path = os.path.join(args.save, args.arch, args.mode, 'checkpoint.pkl')

    if os.path.exists(continue_path):
        checkpoint = megengine.load(continue_path)
        model.load_state_dict(checkpoint['state_dict'])
        opt.load_state_dict(checkpoint['opt'])
        start_step = checkpoint['epoch'] * steps_per_epoch
    ...
    def adjust_learning_rate(step):
        if args.mode == 'normal':
            lr = args.lr * 0.1 ** bisect.bisect_right(
                [30 * steps_per_epoch, 60 * steps_per_epoch * 2, 80 * steps_per_epoch], step
            )
        else:
            lr = args.lr * 0.1 ** bisect.bisect_right(
                [5 * steps_per_epoch, 8 * steps_per_epoch * 2], step
            )
        if step < args.warmup_epochs * steps_per_epoch:  # warmup
        ...
        if (step + 1) % steps_per_epoch == 0:
            ...
            if dist.get_rank() == 0:
                megengine.save(
                    ...
                    os.path.join(args.save, args.arch, args.mode, "checkpoint.pkl"),
                )
...
def build_dataset(args):
    train_dataset = ImageNet(args.data, train=True)
    ...
    valid_dataset = ImageNet(args.data, train=False)
